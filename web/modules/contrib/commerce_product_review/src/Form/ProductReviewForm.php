...
class ProductReviewForm extends ContentEntityForm {
   ...
   */
  public function __construct(EntityRepositoryInterface $entity_repository, EntityTypeBundleInfoInterface $entity_type_bundle_info, TimeInterface $time, AccountInterface $current_user, EntityTypeManagerInterface $entity_type_manager) {
    parent::__construct($entity_repository, $entity_type_bundle_info, $time);

    $this->currentUser = $current_user;
    $this->productReviewTypeStorage = $entity_type_manager->getStorage('commerce_product_review_type');
  }
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    if (!empty($form['published_as']) && empty($form['published_as']['widget'][0]['value']['#default_value'])) {
